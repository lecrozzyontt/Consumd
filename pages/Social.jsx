...
function ThreadCard({ thread, currentUser, onLike, onDelete, onCommentSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setCommentText(''); setReplyTo(null); setSubmitting(false);
  };
    ...
    <article className="thread-card">
      ...
      <div className="thread-actions">
        <button className={`thread-action-btn ${liked ? 'liked' : ''}`} onClick={() => onLike(thread.id, liked)}>
          <span>{liked ? '♥' : '♡'}</span><span>{thread.likes_count ?? 0}</span>
        </button>
        <button className="thread-action-btn"
          onClick={() => { setExpanded(e => !e); setTimeout(() => inputRef.current?.focus(), 50); }} title="Comment">
      ...
      {expanded && (
        <div className="thread-comments">
          ...
          {topLevel.map(c => (
            <div key={c.id} className="thread-comment">
              ...
              <div className="tc-body">
                <div className="tc-top"><span className="tc-username">@{c.username}</span><span className="tc-time">{timeAgo(c.created_at)}</span></div>
                <p className="tc-text">{c.content}</p>
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                  <button className="tc-reply-btn" onClick={() => handleReply(c)}>Reply</button>
                  {currentUser?.id !== c.user_id && <CommentReportButton commentId={c.id} reportedUserId={c.user_id} />}
                </div>
                {replies.filter(r => r.parent_id === c.id).map(r => (
                  <div key={r.id} className="thread-comment thread-comment--reply">
                    ...
                    <div className="tc-body">
                      <div className="tc-top"><span className="tc-username">@{r.username}</span><span className="tc-time">{timeAgo(r.created_at)}</span></div>
          ...
          <form className="tc-form" onSubmit={handleSubmit}>
            {replyTo && <div className="tc-reply-banner">Replying to @{replyTo.username}<button type="button" onClick={() => setReplyTo(null)}>✕</button></div>}
            <div className="tc-input-row">
              ...
              <button className="tc-submit" type="submit" disabled={submitting || !commentText.trim()}>{submitting ? '…' : '↑'}</button>
            </div>
          </form>
        </div>
      )}
      <ReportModal isOpen={reportOpen} onClose={() => setReportOpen(false)} contentId={thread.id} contentType="thread" reportedUserId={thread.user_id} />
    </article>
  );
}
  ...
  return (
    <>
      <button onClick={() => setOpen(true)}
        style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.75rem', color: 'rgba(255,255,255,0.3)', padding: '0' }}>
        Report
      </button>
      <ReportModal isOpen={open} onClose={() => setOpen(false)} contentId={commentId} contentType="comment" reportedUserId={reportedUserId} />
    </>
  );
}
...
export default function Social() {
  ...
  useEffect(() => {
    const handle = () => {
      ...
      setTimeout(() => { silentRefreshSocial(); silentRefreshThreads(); }, 150);
    };
  ...
  const fetchSocialData = useCallback(async () => {
    ...
    const fri  = rows.filter(r => r.status === 'accepted').map(r => ({
      friendship_id: r.id, ...(r.requester_id === user.id ? r.addressee : r.requester),
    }));

    const { data: myGroups } = await supabase.from('group_chat_members').select('group_id').eq('user_id', user.id);
    const ids = myGroups?.map(m => m.group_id) || [];
    let groups = [];
    if (ids.length > 0) {
      const { data: groupData } = await supabase.from('group_chats')
        .select('id, name, created_at, group_chat_members(user_id)').in('id', ids);
      groups = groupData || [];
    }

    if (!mounted.current) return;
    setRequests(req); setOutgoing(out); setFriends(fri); setGroupChats(groups);
    cacheSet('social:friends', { requests: req, friends: fri, outgoing: out, groupChats: groups });
  }, [user?.id]);

  // ─── Load social with spinner ────────────────────────────────
  const loadSocial = useCallback(async () => {
    if (!mounted.current) return;
    setLoadingMessages(true);
    setMessagesError(false);
    try { await withTimeout(fetchSocialData()); }
    catch (e) { console.error('[Social] loadSocial failed:', e); if (mounted.current) setMessagesError(true); }
    finally { if (mounted.current) setLoadingMessages(false); }
  }, [fetchSocialData]);

  // ─── Silent social refresh (tab switch) ─────────────────────
  const silentRefreshSocial = useCallback(async () => {
    try { await withTimeout(fetchSocialData()); }
    catch (e) { console.error('[Social] silentRefreshSocial failed (silent):', e); }
  }, [fetchSocialData]);

  // ─── Core threads fetcher ────────────────────────────────────
  const fetchThreadsData = useCallback(async () => {
    const { data: threadRows, error: tErr } = await supabase
      .from('threads').select('id, user_id, content, likes_count, created_at')
      .order('likes_count', { ascending: false }).order('created_at', { ascending: false }).limit(50);
    if (tErr) throw tErr;
    if (!threadRows || threadRows.length === 0) { if (mounted.current) setThreads([]); cacheSet('social:threads', []); return; }

    const threadUserIds = [...new Set(threadRows.map(t => t.user_id))];
    const [{ data: threadProfiles }, { data: myLikes }, { data: comments }] = await Promise.all([
      ...
      supabase.from('thread_comments').select('id, thread_id, user_id, content, parent_id, created_at')
        .in('thread_id', threadRows.map(t => t.id)).order('created_at', { ascending: true }),
    ]);
  ...
  // ─── Load threads with spinner ───────────────────────────────
  const loadThreads = useCallback(async () => {
    if (!mounted.current) return;
    setLoadingThreads(true);
    setThreadsError(false);
    try { await withTimeout(fetchThreadsData()); }
    catch (e) { console.error('[Social] loadThreads failed:', e); if (mounted.current) setThreadsError(true); }
    finally { if (mounted.current) setLoadingThreads(false); }
  }, [fetchThreadsData]);

  // ─── Silent threads refresh (tab switch) ────────────────────
  const silentRefreshThreads = useCallback(async () => {
    try { await withTimeout(fetchThreadsData()); }
    catch (e) { console.error('[Social] silentRefreshThreads failed (silent):', e); }
  }, [fetchThreadsData]);
  ...
  async function likeThread(threadId, currentlyLiked) {
    setThreads(prev => prev.map(t => t.id === threadId
      ? { ...t, liked_by_me: !currentlyLiked, likes_count: (t.likes_count || 0) + (currentlyLiked ? -1 : 1) } : t));
    if (currentlyLiked) {
      ...
      if (thread && thread.user_id !== user.id) {
        await supabase.from('notifications').insert({ user_id: thread.user_id, actor_id: user.id, type: 'thread_like', reference_id: thread.id, message: `@${user.user_metadata?.username || 'Someone'} liked your thread` });
      }
    }
  }
  ...
  async function submitComment(threadId, content, parentId) {
    ...
    const { data } = await supabase.from('thread_comments')
      .insert({ thread_id: threadId, user_id: user.id, content: filtered, parent_id: parentId || null })
      .select('id, thread_id, user_id, content, parent_id, created_at').single();
    if (data) {
      setThreads(prev => prev.map(t => t.id === threadId ? { ...t, comments: [...t.comments, { ...data, username: user.user_metadata?.username || 'You' }] } : t));
      const thread = threads.find(t => t.id === threadId);
      let targetUserId = thread?.user_id;
      if (parentId) { const parent = thread?.comments?.find(c => c.id === parentId); if (parent) targetUserId = parent.user_id; }
      if (targetUserId && targetUserId !== user.id) {
        await supabase.from('notifications').insert({ user_id: targetUserId, actor_id: user.id, type: parentId ? 'reply' : 'thread_comment', reference_id: threadId, message: `@${user.user_metadata?.username || 'Someone'} ${parentId ? 'replied to your comment' : 'commented on your thread'}` });
      }
      await loadThreads();
    }
  }
    ...
    searchTimeout.current = setTimeout(async () => {
      try {
        ...
        if (thr?.length > 0) {
          ...
          const likedSet = new Set((myLikes || []).map(l => l.thread_id));
          if (mounted.current) setThreadSearch(thr.filter(t => !blockedIds.includes(t.user_id)).map(t => {
        ...
        } else { if (mounted.current) setThreadSearch([]); }
      } catch (e) { console.error('[Social] search failed:', e); }
    }, 350);
  };
  ...
  const sendRequest   = async (targetId) => {
    ...
    if (!error) { showToast('Friend request sent!'); await supabase.from('notifications').insert({ user_id: targetId, actor_id: user.id, type: 'friend_request', message: `@${user.user_metadata?.username || 'Someone'} sent you a friend request` }); loadSocial(); }
  };
  ...
  return (
    <div className="social-page page-wrapper fade-in">
      <div className="social-header">
        <div><h1 className="page-title">Social</h1><p className="page-subtitle">Connect, message, and share thoughts.</p></div>
      ...
      <div className="social-columns">
        ...
        <div className="social-col">
          <div className="col-header">
            ...
            {(friends.length + groupChats.length) > 0 && <span className="count-badge">{friends.length + groupChats.length}</span>}
          </div>
                ...
                <div className="user-list">
                  {searchResults.map(u => (
                    <div key={u.id} className="user-card">
                      <Link to={`/user/${u.id}`} className="user-avatar-link"><Avatar url={u.avatar_url} username={u.username} size={34} className="user-avatar" /></Link>
                      <Link to={`/user/${u.id}`} className="user-name-link"><span className="user-name">@{u.username}</span></Link>
                      <div className="user-action">
                        {isFriend(u.id) ? <span className="badge badge--friend">Friends</span>
                          : hasPendingOut(u.id) ? <span className="badge badge--pending">Pending</span>
                ...
                <div className="user-list">
                  {requests.map(r => (
                    <div key={r.id} className="user-card">
                      <Link to={`/user/${r.requester?.id}`} className="user-avatar-link"><Avatar url={r.requester?.avatar_url} username={r.requester?.username} size={34} className="user-avatar" /></Link>
                      <Link to={`/user/${r.requester?.id}`} className="user-name-link"><span className="user-name">@{r.requester?.username}</span></Link>
                ...
                <div className="user-list">
                  {groupChats.map(g => (
                    <div key={g.id} className="user-card group-chat-card">
                      ...
                      <div className="user-info"><span className="user-name">{g.name}</span><span className="group-members-count">{g.group_chat_members?.length || 0} members</span></div>
                      <button className="btn-message" onClick={() => navigate(`/group-chats/${g.id}`)}>Open</button>
                    </div>
                  ))}
                  {friends.map(f => (
                    <div key={f.id} className="user-card">
                      <Link to={`/user/${f.id}`} className="user-avatar-link"><Avatar url={f.avatar_url} username={f.username} size={34} className="user-avatar" /></Link>
                      <Link to={`/user/${f.id}`} className="user-name-link"><span className="user-name">@{f.username}</span></Link>
        ...
        <div className="social-col">
          ...
          <div className="col-scroll">
            {!searchQuery && (
              <div className="thread-compose">
                ...
                <div className="thread-compose-right">
                  <textarea className="thread-compose-input" placeholder="What are you thinking about?" value={newThreadText} onChange={e => setNewThreadText(e.target.value)} rows={2} maxLength={500} />
                  <div className="thread-compose-footer">
                    <span className="thread-compose-count">{newThreadText.length}/500</span>
                    <button className="btn-post-thread" onClick={postThread} disabled={postingThread || !newThreadText.trim()}>{postingThread ? '…' : 'Post'}</button>
                  </div>
                </div>
              </div>
            )}
            ...
            {loadingThreads ? (
              ...
              <div className="empty-state-col"><p>{searchQuery ? 'No threads match your search.' : 'No threads yet. Be the first to post!'}</p></div>
            ) : (
              <div className="threads-list">
                {visibleThreads.map(t => (
                  <ThreadCard key={t.id} thread={t} currentUser={user} onLike={likeThread} onDelete={deleteThread} onCommentSubmit={submitComment} />
                ))}
              </div>
            )}
          </div>
